    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="min-w-[140px] text-center text-sm font-semibold">{MONTH_NAMES[month]} {year}</span>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {items.length === 0 ? (
            <div className="rounded-lg border border-border p-12 text-center text-muted-foreground">Nenhum item neste mês.</div>
          ) : items.map((item) => {
            const cfg = getTypeConfig(item)
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-3 space-y-1.5">
      ...
      <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
        ...
        {item.course_name && <span className="flex items-center gap-1"><BookOpen className="h-3 w-3" />{item.course_name}</span>}
        {item.turma_name && <span className="flex items-center gap-1"><GraduationCap className="h-3 w-3" />{item.turma_name}</span>}
        {item.subject_name && <span className="flex items-center gap-1"><GraduationCap className="h-3 w-3" />{item.subject_name}</span>}
        {item.professor_name && <span className="flex items-center gap-1"><GraduationCap className="h-3 w-3" />Prof. {item.professor_name}</span>}
      </div>
